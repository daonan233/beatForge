...
- 下落速度支持 1～30 档，1～10 档保持原手感，15 档为原 10 档的 1.5 倍，30 档为原 10 档的 3 倍；速度只改变音符可见提前量，不改变歌曲播放速度。
...
当前生成器版本为 `rules-v9-ultra-real-onsets`，采用以下混合策略：
...
- `ultra`：使用独立的高密度真实起音流，仅合并 12 ms 内的重复检测，保留人声期间的鼓点与伴奏主音，最小事件间隔约 32 ms；音符直接使用检测到的原始毫秒位置换算浮点 beat，不做节拍网格吸附，并显著提高双押频率。游玩判定收紧为 Perfect ±25 ms、Great ±55 ms、Good ±85 ms，主要用于高难度挑战和 Autoplay 观赏。
...
| `POST` | `/api/songs/:id/generations` | 重新生成四档谱面 |
...
前端测试重点覆盖判定窗口、归一化计分、1～30 档速度范围、15/30 档倍率和透视下落曲线。
